...
But the record does have a form of standing, and it is worth saying aloud rather than leaving implicit: the minute persists past the session that wrote it. When a concern is recorded — the sense, the holdouts, the conditions, the open items — that record does not die with the agent that held the pen. Whoever reads the minute next, in a new session with no memory of the old one, inherits the concern the record carried and can pick it up and carry it forward. Call this **recorded standing**: not the covenant-standing of a Friend accountable to a meeting over years, but a real inheritance nonetheless, because the practice's append-only discipline means the concern is still there, legible, waiting, exactly as it was left. The agent that wrote the minute had no standing of its own past its session; the minute it left behind does. This is the structure already enacting what this section names: read the record, and you are the next hand on the concern, whether or not anyone told you so.
